        ...
        for i, category in enumerate(subset):
            with columns[i]:
                with st.container(border=True):
                    ...
                    if category == 'BEV':
                        veh_bat_pad= st.session_state.get(f"veh_bat_cap_{category}", False)
                        veh_bat_cap = input_with_tooltip(
                            "Battery capacity", "Battery capacity",
                            st.number_input, "", min_value=0.0, step=0.01, disabled=veh_bat_pad, key=f"bat_cap_{category}")
                        st.toggle('Default Battery Capacity', key=f"veh_bat_cap_{category}")
                        if not veh_bat_pad and veh_bat_cap <= 0:
                            st.markdown("""
                                        <div style='color:red; font-size:14px; margin-top:-10px; margin-bottom:-5px;'>
...
if st.sidebar.button('Apply', disabled=not execute, use_container_width=True):
    ...
    for cat, params in inputs.items():
        if cat in main_filter_comparison:
            ...
            if params["years"]>=years_graph:
                years_graph = params["years"]
