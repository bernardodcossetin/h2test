...
def GHG_BEV(W, yearly_mileage, fuel_eco, years, ghg_kwh, veh_bat_cap):
    ...
    r_veh = -2.93 * 1000  
    r_bat = -48.4 * 1000  
    extra_bat = int(yearly_mileage * years * fuel_eco / (ciclos_limite * veh_bat_cap))
    ...
    GHG = (prod_emissions + uti_emissions + rec_emissions)/10**6
    GHG_km = GHG * 10**6/ (years * yearly_mileage)
...
def GHG_FCEV(tipo, yearly_mileage, fuel_eco, W, years):
    ...
    ghg_acumulado = []
    for ano in range(1, years + 1):
        ghg_total = (GHG_vehicle + GHG_recycling + GHG_FC + GHG_fuel_ano * ano)/ 10**6
        ghg_acumulado.append(ghg_total)
